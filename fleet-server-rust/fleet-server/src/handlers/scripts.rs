...
pub async fn run_script(Json(_body): Json<RunScriptBody>) -> FleetResponse {
    fleet_ok("execution_id", serde_json::json!(""))
}

/// POST /api/_version_/fleet/scripts/run/sync
pub async fn run_script_sync(Json(_body): Json<RunScriptSyncBody>) -> FleetResponse {
    fleet_ok("", serde_json::json!({}))
}

/// POST /api/_version_/fleet/scripts/run/batch
pub async fn batch_script_run(Json(_body): Json<BatchScriptRunBody>) -> FleetResponse {
    fleet_ok("batch_execution_id", serde_json::json!(""))
}

/// GET /api/_version_/fleet/scripts/results/{execution_id}
pub async fn get_script_result(Path(_execution_id): Path<String>) -> FleetResponse {
    fleet_ok("", serde_json::json!({}))
}

/// POST /api/_version_/fleet/scripts
pub async fn create_script() -> FleetResponse {
    // TODO: handle multipart upload of script content
    fleet_ok("script", serde_json::json!({}))
}

/// GET /api/_version_/fleet/scripts
pub async fn list_scripts(Query(_params): Query<ListScriptsParams>) -> FleetResponse {
    fleet_ok("scripts", serde_json::json!([]))
}

/// GET /api/_version_/fleet/scripts/{script_id}
pub async fn get_script(Path(_script_id): Path<u64>) -> FleetResponse {
    fleet_ok("script", serde_json::json!({}))
}

/// PATCH /api/_version_/fleet/scripts/{script_id}
pub async fn update_script(Path(_script_id): Path<u64>) -> FleetResponse {
    // TODO: handle multipart upload
    fleet_ok("script", serde_json::json!({}))
}

/// DELETE /api/_version_/fleet/scripts/{script_id}
pub async fn delete_script(Path(_script_id): Path<u64>) -> FleetResponse {
    fleet_ok("", serde_json::json!({}))
}

/// POST /api/_version_/fleet/scripts/batch
pub async fn batch_set_scripts(Json(_body): Json<BatchSetScriptsBody>) -> FleetResponse {
    fleet_ok("", serde_json::json!({}))
}

/// POST /api/_version_/fleet/scripts/batch/{batch_execution_id}/cancel
pub async fn batch_script_cancel(Path(_batch_execution_id): Path<String>) -> FleetResponse {
    fleet_ok("", serde_json::json!({}))
}
