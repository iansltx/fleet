...
VALUES (?, ?, ?, ?, UNHEX(SHA2(mobileconfig, 256)))`
...
`)
	...
	r, err := db.Exec(stmt, 0, "com.fleetdm.fleet.mdm.filevault", "Disk encryption", mcBytes)
	...
	r, err = db.Exec(stmt, teamID, "com.fleetdm.fleet.mdm.filevault", "Disk encryption", mcBytes)
	require.NoError(t, err)
	...
	verifyNewPayload := func(profileID int64) {
		...
		require.EqualValues(t, 1, found["DeferForceAtUserLoginMaxBypassAttempts"])
	}
